...
| `CRAWLER_LISTS_DRAIN_CAPS` | *(none)* | Company lists: csv of `siteId:cap` — the per-pass drain budget, and the switch: a store absent or at `0` runs no lists step at all. Only id-range stores (`CRAWLER_RANGE_STORES`). The drain also spends the global `CRAWLER_MAX_REQUESTS` ahead of the older gaps, the descent and re-observation: raise that by the drain cap when arming |
...
- **Frontier** — the highest itemId in the store's ledger that reads as a positive integer (never
  one the gap sweep wrote), else the operator's `CRAWLER_RANGE_FRONTIER_<SITEID>` seed. With neither, the walk is skipped with a WARN
...
- **Cursor** — `range.cursor` in the ledger is the next id to walk, and it moves DOWN only by the
  ...
  skew) leaves the cursor exactly where it was, with a WARN and `rangeSkipped: "window-rejected"`:
  that band would otherwise be spent collecting nothing and is never walked again. A window that also
  held ids already in the ledger is not refused entirely: the cursor moves past it, and each refused
  id keeps its own row in the fetch-failure ledger.
...
- **Re-anchor** — once per `CRAWLER_RANGE_REANCHOR_H` (default 24 h; `0` = every run) the frontier is
  moved up to the highest numeric id the store's LEDGER holds that the gap sweep did NOT write. Above
  the frontier that is what a listing (the tap) has shown us: every id the sweep writes carries
  `sweptFrom` (the origin of its band), because it came from a synthesized byId window and
  `/ingest/scrape` accepts such a url whether or not the item exists. The band between the old
  frontier and the new one is recorded as a KNOWN GAP. It costs no request (it reads the ledger the
  pass already loaded) and it is persisted on its own, so it still happens on a run whose descent does
  nothing at all. The run summary reports `rangeReanchoredTo`: the id the frontier MOVED to this run,
  `null` when it did not move. A move wider than `CRAWLER_RANGE_REANCHOR_MAX_DELTA` is refused with a
  WARN and reported as `rangeReanchorRefused`. A store that has never walked has no frontier to move;
  the descent seeds one instead.
- **Known-gap bands** — `range.gaps` in the ledger, beside the cursor: `{ from, to, next, origin,
  ...
  (`mfc:3700000-3700400,mfc:123456` — a single id is a band of width 1); each is adopted ONCE, matched
  ...
  and adopted nowhere. A declared band must lie at or below the store's frontier (checked after this
...
- **Sweep** — `CRAWLER_RANGE_GAP_BUDGET` ids per store per run (default `0` = OFF), `reanchor` bands
  ...
  ENTIRELY leaves `next` exactly where it was, exactly as the descent leaves its cursor, and reports
  `gapSkipped: "window-rejected"`. A window that also held ids already in the ledger advances past
  its refused ids, each of which keeps its own row in the fetch-failure ledger: a re-anchor band is
  mostly known ids by construction, and holding its cursor would re-drive the same refusal every run.
...
- **Reporting** — `gapIdsSwept` (ids the band cursor MOVED over — a window the store refused entirely
  ...
  `unsupported`, `failed`, `window-malformed`, `window-rejected` — `null` on a run that actually swept. Run level:
...
- **Stop switch** — remove the store from `CRAWLER_LISTS_DRAIN_CAPS` (no fetch, no drain); unset
  ...
  lane lands WARM in FIFO order; an older engine answers `/catalog/rotating` 404 and nothing is fetched.
...
- `BROWSER_LAUNCH_MODE`: `clean-headful` selects the proven Cloudflare-passing launch profile (real Chrome, headful on the Ozone headless platform, no automation switch, minimal flags) — see **The browser lane** below
  ...
  - Default: unset
- `BROWSER_NAV_TIMEOUT_MS`: Navigation budget (ms) for every browser-lane `page.goto` — the target fetch and the session prime alike
  - A Cloudflare-fronted store reached through a relayed residential exit spends most of a navigation on the CHALLENGE, not on bytes: production 2026-09-09 booked 65 `Navigation timeout of 20000 ms exceeded` on anitoys and sugotoys while the path itself measured 1.4 MB/s with sub-second TTFB
  - Unset/blank → the default, silently; set but non-numeric or non-positive → the default with ONE boot warning; any usable value is clamped to `[5000, 120000]`
  - A store may override it for itself with `searchFetch.navTimeoutMs` (contract 0.11.0), clamped the same way — prefer that to raising the whole pod's budget for one slow store
  - The effective value is published as `browserLane.navigationTimeoutMs` on `GET /health/detailed`
  - Default: `20000`
...
**Which lane an image takes** is decided by the policy table first, then by the engine's existing suffix rule. An image on the DECLARING store's own hosts inherits that store's transport and egress; anything else has the declared egress DROPPED (the residential exit is a home line — it is not spent on a host that never declared it) and takes the plain lane. The table OVERRIDES both directions. Three pairings are refused outright rather than resolved, and none is silently downgraded — a downgrade answers a misconfigured row with a different symptom somewhere else:
...
- **`lane: browser` + `ua: default`** (`browser-lane-default-ua`) — `default` means "claim no browser", and a browser tab cannot: its user agent is the browser's own Chrome string whatever the row says. Resolving it to Chrome would invert the one decision the operator made, so the loader warns on such a row at boot (`[IMAGE-POLICY]`) and the decision refuses every image that would ride it — including one whose row names no lane and inherits the browser lane from the store, which, like off-store, no table can know. A host that needs `default` needs `http` or `impit`.
...
| `ua` | `chrome` \| `default` | User-agent profile, resolved to a header by `resolveImageUserAgent` (`chrome` = the desktop Chrome string, whose major tracks the engine's impersonation profile; `default` = no browser claim: no user agent at all on `http`; on `impit`, whatever that lane already sends — its impersonation profile's UA, or the mint UA of a host with hand-minted cookies, which always wins there; the browser lane cannot honour it, and the pairing is refused — see above). Default: `chrome` on the impit/browser lanes, `default` on `http` |
...
- `RAW_STORE_CONCURRENCY`: how many store ops (a HEAD or a PUT) the sink runs at once, **across both lanes**. Captures beyond it wait in the sink's own queue rather than opening another socket. Unset/invalid → the default; clamped to `64` so a typo cannot restore the unbounded fan-out. Default: `4`
- `RAW_STORE_QUEUE_MAX_BYTES`: how many BYTES those waiting captures may hold, checked alongside the depth — whichever ceiling binds first drops the capture, and the two are counted apart (`dropped` vs `droppedBytes`) so an operator can see which one it was. A count is not a memory bound: the same 500-deep queue is ~15 MB of page bodies or up to ~5 GB of 10 MiB originals, against a 3Gi container with 2Gi of it spoken for by Chrome's `/dev/shm`. Unset/invalid → the default. Default: `268435456` (256 MiB)
- `RAW_STORE_QUEUE_MAX`: how many captures may WAIT for a slot. Beyond it a capture is **dropped** — counted as `rawStore.stats.dropped`, reported in one log line per minute (never per drop), and *told to the caller*: `capture()` answers `{admitted: false, reason: 'queueFull' | 'queueBytesFull'}` so a caller that keeps a per-url memo does not record a lost capture as done. The image capture hook acts on exactly that: a refused capture counts as `failed` with the named skip `sinkQueueFull`, and the url is **not** memoized, so the next pass retries it. A kill switch or a typed skip is *not* a refusal — the sink resolved the capture, and re-offering it would change nothing. A page body is a buffered `Buffer`, so an unbounded backlog is memory held on a scrape's behalf, and a capture that outruns the store by minutes is stale by the time it would be written. Unset/invalid → the default; clamped to `1`..`5000`, since a depth in the tens of thousands is the unbounded backlog wearing a number. Default: `500`

- `RAW_STORE_ASSET_QUEUE_SHARE`: the share of BOTH queue budgets (depth and bytes) the **asset lane** may occupy. The rest is **reserved for page bodies**, and the reservation exists because the two lanes' losses are not comparable: a page body is the provenance behind a claim already written to the spine and nothing anywhere will fetch it again, while an image is a re-fetchable blob the next crawl pass would pick up regardless. Shared first-come-first-served they are not equal claimants — when the image lane was first armed in prod (2026-09-09) the queue sat on its depth ceiling and refused **1561** captures, **≈63 of them pages**: a permanent provenance gap bought with bytes that were coming back anyway. An asset offered while the queue is at or above its share is refused with `{admitted: false, reason: 'assetReserve'}` — a refusal like any other, so the image hook still counts it `sinkQueueFull`, leaves the url un-memoized and retries next pass — and counted apart **by the budget that held it**: `rawStore.stats.assetRefusedReserveDepth` / `assetRefusedReserveBytes` (`assetRefusedReserve` is their sum), mirroring `dropped` vs `droppedBytes` and for the same reason — a depth problem wants `RAW_STORE_QUEUE_MAX` raised, a payload problem `RAW_STORE_QUEUE_MAX_BYTES`. Both feed **one** once-a-minute log line (`asset capture held back for the page reservation`) — one line per window, naming the budget that opened it and that knob, with the running totals for **both** budgets on every line, so a burst that bound on both still reads right and "the store is behind" never reads as "the reservation is working" — and with the queue depth **by lane** (`N queued = P page(s) + A asset(s)`), because the share is a share of the whole queue: pages can spend it with no asset waiting at all, and that must not read as an image backlog. The two budgets are measured differently, because their units are: **depth** is tested against what the queue holds *now* (a one-slot overshoot is one slot), while **bytes** are tested against what the queue *would* hold once this capture is added. One asset can be up to `RAW_STORE_IMAGE_MAX_BYTES` — settable as high as 64 MiB, which is exactly the quarter of the 256 MiB default budget this reservation holds — so measuring bytes on occupancy alone let four assets walk a queue from just under the share to the hard ceiling and refuse the next **page** with `queueBytesFull`, spending the whole reserve on images. The one exemption is an **empty** queue, and only on the byte side: a capture waiting behind nobody displaces nobody, so it is admitted even when it alone exceeds the share — at most one asset is ever held that way, since the next is measured against it. The full `RAW_STORE_QUEUE_MAX` / `RAW_STORE_QUEUE_MAX_BYTES` ceilings still bound the memory either way. **What the share measures is the queue** — captures *waiting* for a worker, both lanes combined. An upload already running has left the queue and holds no slot in it (the concurrency bounds what runs, the queue bounds what waits), and its bytes sit **outside** `RAW_STORE_QUEUE_MAX_BYTES`. So the sink's footprint is, in captures, `RAW_STORE_QUEUE_MAX` waiting + `RAW_STORE_CONCURRENCY` running, and in bytes `RAW_STORE_QUEUE_MAX_BYTES` + `RAW_STORE_CONCURRENCY × RAW_STORE_IMAGE_MAX_BYTES` (the most one running upload can hold): at `RAW_STORE_CONCURRENCY=12` as deployed that is 500 + 12 captures and 256 MiB + 12 × 10 MiB = **376 MiB**, of which the asset lane's share is 375 + 12 captures and 192 MiB + 120 MiB = 312 MiB. On `/health/detailed` that is `queued` (split as `queuedPages` / `queuedAssets`) beside `inFlight`, and `queuedBytes` beside the in-flight bytes it does not count — and an empty queue admits an asset however many images are uploading. Counting the running uploads against the share was tried and rejected: at `RAW_STORE_QUEUE_MAX=8` with `RAW_STORE_CONCURRENCY=8` it refuses the seventh asset with the queue empty and two workers idle. **Pages go first**: the two lanes wait in two queues and a worker takes a waiting page before any waiting asset (FIFO inside each lane), so a page admitted into the reserved tail waits behind at most one running upload — plus one aged asset when that lane's turn has come, see `RAW_STORE_ASSET_MAX_WAIT_MS` — rather than the whole image backlog. The budgets and the share are measured over the two queues combined, so priority changes no single admission decision, only who goes *next*. What it does change over time is which lane's captures stay *resident*: while pages keep coming the assets do not drain, so they keep holding their slots and their bytes, and a page can be refused `queueBytesFull` against bytes a waiting asset holds where first-come-first-served would have drained it and given them back — the page reserve `(1 − share) × RAW_STORE_QUEUE_MAX_BYTES` holds either way; what priority costs a page is the opportunistic headroom above it. `1` turns the reservation off (the old first-come-first-served admission; pages still go first). Unset/invalid → the default; clamped to `1` from above, and a `0`/negative value falls back to the default rather than closing the lane, because turning the asset lane off is `PERSIST_RAW_IMAGES`' job. Default: `0.75`
- `RAW_STORE_ASSET_MAX_WAIT_MS`: how long an asset may wait behind pages before the asset lane's oldest capture goes next. Strict pages-first let a crawler burst (~17 minutes of every hour in prod) park the whole resident asset backlog for its duration — nothing uploaded, every new asset refused `assetReserve`, and the hold line naming `RAW_STORE_QUEUE_MAX`, a knob that frees nothing because those captures were short of a worker that prefers pages, not of space. So an asset that has waited **longer than** this window goes next — **one** of them, then a page again. That alternation is the bound: out of a page burst the asset lane gets at most every other take, a page never waits behind more than one asset, and the window bounds how long the lane waits for its next *turn*, not how long every asset waits (a 375-deep backlog still drains one asset per page). The shutdown flush is exempt and stays strictly pages first. Unset/invalid → the default; clamped to `1000` from below, since a window of a few milliseconds is round-robin wearing a number. Default: `30000`

- `RAW_STORE_SHUTDOWN_FLUSH_MS`: how long `SIGTERM`/`SIGINT` waits for the queue to reach the store before exiting. Those captures were *accepted*, and they are write-once bytes with nothing behind them that would fetch the page again, so discarding them in silence is a real loss — but the wait is bounded, because a pod that will not exit is worse. The drain is **pages first**, so the budget goes to the captures nothing will fetch again and what the bound abandons is, as far as the backlog allows, images the next pass re-fetches. Priority reorders only what is *waiting*, though: a budget shorter than one PUT's p95 cannot rescue a page when every worker is mid-PUT at `SIGTERM`, because no worker frees inside it — the deployed `8000` against a measured PUT p95 of ~13 s is exactly that case, and it is a tuning note for the deployment's value, not something the sink can fix. Whatever could not be flushed is named in one log line. Default: `8000`

  The queue also closes the write-once race that concurrency opened: two captures of the same content address in flight together would both HEAD-miss and both upload the same object, so an address with an op already running is treated as a dedup rather than a second write. Both lanes share that guard — their prefixes keep the key spaces apart.

  Why the queue exists: capture used to issue its HEAD+PUT pair the moment a page was captured, so an 18-store crawl wave put hundreds of ops in flight from one pod at once. They then contended for DNS-lookup slots on libuv's 4-thread pool, for TLS handshakes, and for an event loop that the synchronous gzip was blocking — and the per-op budget, which starts when the op is issued, was spent on that contention rather than on the upload. `capture()` now ADMITS and returns (the fetch path never waits on the bucket), compression runs off the event loop, and `RAW_STORE_PUT_TIMEOUT_MS` / `RAW_STORE_IMAGE_PUT_TIMEOUT_MS` bound **the capture's whole trip to the store**. The wait for a slot is reported separately as `queueWaitP50`/`queueWaitP95` beside `headP50`/`headP95` and `putP50`/`putP95` on `/health/detailed`, so a slow lane can be read as a slow bucket or as a backlog behind a healthy one. Both op percentiles sample **failures as well as successes**: an op that spent its whole budget is precisely the one an operator needs in the tail, and a percentile built from successes only lets a lane that is timing out on every upload report the same fast p95 as a healthy one.

  **The budget cancels, and it covers the whole op.** Reaching `RAW_STORE_PUT_TIMEOUT_MS` used to mean only that the sink stopped *waiting*: the request kept its socket and kept spending the store's request budget, beside the replacement op the freed worker had already started — so `RAW_STORE_CONCURRENCY` bounded what the sink was *watching*, not what was in flight, and every op that overran made the next likelier to overrun too. The budget now **aborts the request at the socket**, so a bound op really does stop costing the store anything, and the raw-store client keeps its own connection pool capped at `RAW_STORE_CONCURRENCY` rather than sharing the process-wide agent. Two consequences an operator has to know. First, the budget is spent across the **whole capture** — its HEAD, its gzip and its PUT together, not each of them: `RAW_STORE_PUT_TIMEOUT_MS=30000` is now a true 30 s, where the per-call reading of the same number bought a 60 s worst case (and the asset lane's 60 s a 120 s one). Second, reaching it **loses the body**, where before the PUT usually still landed and was merely mis-counted. That is the better failure for a provenance store — the bucket and the counters now agree, instead of the bucket holding objects nothing recorded — but it means the budget belongs comfortably above the store's measured cost, not near it. A page body is the capture nothing will ever re-fetch; an image is the one the next pass picks up anyway, which is why the asset lane may carry the longer budget despite being the cheaper loss.

  **`timedOut`** counts the ops the budget ended — a **subset** of `failed` + `assetFailed`, not a separate bucket. "The store answered with an error" and "the store never answered" are different faults wanting different responses, and until this counter existed they arrived on the health page as the same number. It spans **both lanes**, so a rising count does not by itself say whether the page budget or the asset budget is the one being hit; read it against which of `failed` / `assetFailed` moved with it.

  **`eventLoopLagP50` / `eventLoopLagP95` / `eventLoopLagMax`** are this process's own scheduling delay, sampled four times a second while the sink has work. They are here because every latency above is wall clock read **inside** this process, so anything that keeps the process off the CPU — a long synchronous parse, a co-tenant Chrome, a cgroup CPU quota — inflates them all without the store having slowed down at all. **Read them together with `putP95`: a fat `putP95` over a flat lag is the store; a fat `putP95` over a fat lag is this pod.** `eventLoopLagMax` is a high-water mark since process start and is not redundant with the percentiles: the sampler resets its due time after each reading, so one stall of duration D leaves exactly **one** sample of ~D, and a lone sample does not move a p95 built over a 512-reading window — a 3 s stall can sit under `eventLoopLagP95: 0`. Sustained throttling *does* show in the percentiles (100 ms slices stall most 250 ms samples); a single long stall shows only in the max, and the max is what answers "was this pod stalled while that 29 s PUT was outstanding?".

  **`cpuThrottling`** (a sibling block on `/health/detailed`, not part of `rawStore`) is the kernel's own answer, read from cgroup v2 `/sys/fs/cgroup/cpu.stat`: `nrPeriods`, `nrThrottled` and `throttledUsec`. A fat lag says *this process* was slow; it cannot say whether that was the process's own doing or a quota stopping it, and those want opposite remedies — a code fix versus a limits change. These are counters rather than samples, so nothing hides in a window or below a percentile. They are **cumulative** for the life of the cgroup, which is why the block carries `readAt`: a single reading says little, two polls minus each other say everything. `{available: false}` where there is no cgroup v2 to read (dev boxes, CI); the throttle fields are absent on a cgroup with **no** `cpu.max` limit, which is "no quota set", not "no throttling observed".
- `RAW_STORE_S3_ENDPOINT` / `_REGION` / `_BUCKET` / `_PREFIX` / `_JSON_PREFIX` / `_PATH_STYLE`, `RAW_STORE_KEY_SCHEME` (only `sha256-v1` is understood — an unknown scheme fails fast at boot), `RAW_STORE_PUT_TIMEOUT_MS` (default `30000`, page lanes only — the WHOLE-op budget described above; the default matches the deployed value because a default under the store's measured cost would abort about half of all uploads and destroy those bodies), `RAW_STORE_IMAGE_PUT_TIMEOUT_MS` (default `30000` — the asset lane's own whole-op budget: an original image is up to 10 MiB against a page body's ~30 KB gzipped, so it cannot share the page budget), and the credential pair `RAW_STORE_S3_ACCESS_KEY_ID` / `RAW_STORE_S3_SECRET_ACCESS_KEY`.
