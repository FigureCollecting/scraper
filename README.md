...
- `MFC_SESSION_CANARY_ITEM`: the item id of a KNOWN NSFW+ mfc item, used to tell those two cases apart. One 404 proves nothing; a PAIR does — when the canary answers 404 while a known-good SFW control is served, the session has lost its entitlement, the mfc session is flagged stale on `/health/detailed` (`sessionCanary`) and one `[MFC SESSION]` line is logged on the transition. A canary answering 200 clears the flag; anything else (a failing control, another status) is inconclusive and leaves a standing flag alone. Unset/blank = the canary is off. The two observations are supplied by the caller — nothing here fetches.
