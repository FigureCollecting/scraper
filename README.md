...
| `ua` | `chrome` \| `default` | User-agent profile, resolved to a header by `resolveImageUserAgent` (`chrome` = the desktop Chrome string, whose major tracks the engine's impersonation profile; `default` = no browser claim: no user agent at all on `http`, the impersonation profile's on `impit`; the browser lane cannot honour it, and the pairing is refused — see above). Default: `chrome` on the impit/browser lanes, `default` on `http` |
