...
- `RAW_STORE_QUEUE_MAX`: how many captures may WAIT for a slot. Beyond it a capture is **dropped** — counted as `rawStore.stats.dropped` and reported in one log line per minute, never per drop. A page body is a buffered `Buffer`, so an unbounded backlog is memory held on a scrape's behalf, and a capture that outruns the store by minutes is stale by the time it would be written. Unset/invalid → the default; clamped to `1`..`5000`, since a depth in the tens of thousands is the unbounded backlog wearing a number. Default: `500`
